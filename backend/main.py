...
"""
yo Rishi i updated it to fit the actual api, i had to change parts
i tried to commment out and leave parts incase theyre updated in the future hope u dont mind
"""
...
def _extract_ai_values(data: dict[str,Any]) -> str:
    """
    ...
    try:
        ...
        raise MissingAIFieldError(Completion_key) from error
    if not isinstance(completion, str):
        raise InvalidAIResponseError("completion must be a string")
    completion = normalize_response_text(completion)

    return completion
def parse_ai_response(json_data: str) -> str:
    """
...
def parse_http_response(http_response: requests.Response) -> str:
    """
...
def send_text_to_server(text:str,url:str,max_retires:int=2,retry_delay: float = 0.5,) -> requests.Response:
    ...
    for attempt in range (max_retires + 1):
        try:
            response = requests.post(url,json={"context":text, "is_secret":False}, timeout=10,)
